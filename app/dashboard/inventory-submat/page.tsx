...
export default async function InventorySubmatPage() {
  let rows: Awaited<ReturnType<typeof getSubmatStockRows>> = null
  let loadError: string | null = null
  try {
    rows = await getSubmatStockRows()
  } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold tracking-tight">포장 부자재 재고</h1>
        <p className="text-sm text-muted-foreground">포장 부자재별 현재 재고 수량을 확인하고 수정합니다</p>
      </div>
      ...
      {loadError || rows === null ? (
        <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-5 text-amber-800">
          ...
          <div className="text-sm">
            ...
            <p className="mt-1 text-amber-700">
              tb_submat_mst 테이블에 stock_qty 컬럼이 아직 없을 수 있습니다. 마이그레이션(DDL)을 먼저 실행했는지
              확인해주세요.
              {loadError ? ` (${loadError})` : ""}
            </p>
          </div>
        ...
        <div className="flex flex-col items-center gap-2 rounded-xl border border-border bg-card p-12 text-center text-muted-foreground">
          ...
          <p className="text-sm">등록된 포장 부자재가 없습니다.</p>
        </div>
      ) : (
        <InventoryTable
          rows={inventoryRows}

...
export function DashboardSidebar({
  ...
  const isDataTable = pathname.startsWith(DATA_TABLE_HREF)
  ...
  const navItems = [
    ...
    { label: "데이터 테이블", href: DATA_TABLE_HREF, icon: Database, visible: true },
      ...
      <nav className="flex flex-1 flex-col overflow-y-auto py-3">
        ...
        {navItems
          ...
          .map(({ label, href, icon: Icon, exact }) => {
            ...
            const isTableNavItem = href === DATA_TABLE_HREF
            ...
            return (
              <div key={href}>
                ...
                {/* 데이터 테이블 서브메뉴: 해당 항목 바로 아래에 표시 */}
                {isTableNavItem && isDataTable && (
                  <div className="bg-gray-950/40 py-1.5">
                    {TABLE_SUBMENU_ITEMS.map(({ label: subLabel, table }) => {
                      const isSubActive =
                        pathname === `${DATA_TABLE_HREF}/${encodeURIComponent(table)}`
                      return (
                        <Link
                          key={table}
